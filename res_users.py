...
class res_users(osv.Model):
    ...
    def _signup_create_user(self, cr, uid, values, context=None):
        ...
        values['active'] = True
        res_user_id = self.copy(cr, uid, template_user_id, values, context=context)
        res_user_obj = self.browse(cr,uid,res_user_id)
        self.pool.get('res.partner').write(cr,uid,res_user_obj.partner_id.id,{'user_id':res_user_obj.id})
        ###search template signup
        mail_obj = self.pool.get('mail.mail')
        template = self.pool.get('ir.model.data').get_object(cr, uid, 'auth_signup', 'new_user_email')
        mail_id = self.pool.get('email.template').send_mail(cr, uid, template.id, res_user_id, True, context=context)
        mail_state = mail_obj.read(cr, uid, mail_id, ['state'], context=context)
        if mail_state and mail_state['state'] == 'exception':
            raise osv.except_osv(_("Cannot send email: no outgoing email server configured.\nYou can configure it under Settings/General Settings."), res_user_obj.name)
        return res_user_id
